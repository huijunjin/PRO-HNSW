...
//pro-hnsw
        .def("find_all_components",
             [](Index<float>& self){
                 return self.appr_alg->findAllComponents();
             },
             "Return the list of components"
        )
        .def("get_node_max_level",
             [](Index<float>& self, unsigned int node_id){
                 return self.appr_alg->getNodeMaxLevel(node_id);
             },
             py::arg("node_id"),
             "Return max level"
        )
        .def("get_node_neighbors",
             [](Index<float>& self, unsigned int node_id, int level){
                 return self.appr_alg->getNodeNeighbors(node_id, level);
             },
             py::arg("node_id"), py::arg("level"),
             "Return adjacency list at given level"
        )
		.def("repair_disconnected_nodes",
			 [](Index<float> &self){
				 return self.appr_alg->repairDisconnectedNodes();
			 },
			 "Repairs disconnected nodes and returns the number of nodes processed."
		)
		.def("resolve_edge_asymmetry",
			 [](Index<float> &self){
				 return self.appr_alg->resolveEdgeAsymmetry();
			 },
			 "Resolves edge asymmetry and returns the count of fixed edges."
		)
        .def("remove_obsolete_edges",
            [](Index<float> &self, const std::vector<unsigned int> &delete_nodes){
                return self.appr_alg->removeObsoleteEdges(delete_nodes);
            },
            py::arg("delete_nodes"),
            R"pbdoc(
                Removes edges pointing to deleted nodes, returns count of removed edges.
            )pbdoc"
        )
//end of pro-hnsw
